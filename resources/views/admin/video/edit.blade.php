		...
		<div class="box-body">
			<div class="row">
				<div class="col-md-12">
                    ...
                    {{Form::open(array('route' => array('video.update',$video->id), 'method' => 'PUT', 'class' =>'form-horizontal', 'files' => true))}}
                    {{ csrf_field() }}
                    ...
                    <table class="table table-striped table-bordered table-hover table-sm datatable mdl-data-table dataTable" role="grid" style="width: 80%;">
                           ...
                           <tr>
                            ...
                            <tr>
                                <td>Title:</td>
                                <td>{{ Form::text('title',$video->title ,array('class' => 'form-control','placeholder' => 'Enter Video Title')) }}</td>
                            </tr>
                              <tr>
                                <td>URL:</td>
                                <td>{{ Form::text('url',$video->url ,array('class' => 'form-control','placeholder' => 'Enter Video URL')) }}</td>
                            </tr>
                            <tr>
                                <td>Default (?):</td>
                                <td>{{Form::select('is_default', array( '' => '-----', 1 => 'Yes', 0 => 'No'),$video->is_default,['class' => 'form-control'])}}</td>
                            </tr>
                            <tr>
                                <td>Active (?):</td>
                                <td>{{Form::select('is_active', array( '' => '-----', 1 => 'Yes', 0 => 'No'),$video->is_active,['class' => 'form-control'])}}</td>
                            </tr>
                        </tbody>
                    </table>
                    </div>
